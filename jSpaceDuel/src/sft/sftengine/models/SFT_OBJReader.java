...
public class SFT_OBJReader {
    ...
    public SFT_MaterialLibrary materialLib;
    ...
    public SFT_OBJReader(String objfilename) {  // Construct from file name
        loadobject(objfilename);
    }

    public SFT_OBJReader(InputStream in) {  // Construct from inputstream
        loadobject(in);
    }

    public final void loadobject(String objfilename) {  // load from String filename
        if (objfilename != null && objfilename.length() > 0) {
            // Separate leading path from filename (we'll load material libe from same folder)
            String[] pathParts = SFT_Util.getPathAndFile(objfilename);
            ...
            try {
                ...
                System.out.println("SFT_OBJReader.loadobject(): Failed to read file: " + objfilename + " " + e);
            }
        }
    }
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (line.length() > 0) {
                    if (line.startsWith("v ")) {
                        ...
                        materialLib = new SFT_MaterialLibrary(filepath + materialLibeName);
                    }
                }
            }
        } catch (Exception e) {
     ...
     * @return       Face object
     */
    private SFT_Face readFace(String line) {
    ...
    public void calcDimensions() {
        ...
        for (int g = 0; g < groups.size(); g++) {
            ArrayList FacesL = ((Group) groups.get(g)).faces;
            // loop through all faces in group (ie. triangles)
            for (int f = 0; f < FacesL.size(); f++) {
                SFT_Face face = (SFT_Face) FacesL.get(f);
