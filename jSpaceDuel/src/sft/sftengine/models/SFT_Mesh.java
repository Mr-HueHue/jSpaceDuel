...
public class SFT_Mesh {
    ...
    public SFT_Vertex[] vertices;
    public SFT_Triangle[] triangles;
    ...
    public SFT_Material[] materials = null;
    // For managing triangle groups.  See makeGroups().
    // start with 1 group (default)
    SFT_Triangle[][] groupFaces = new SFT_Triangle[1][];
     ...
     * @return the vertex
     */
    public SFT_Vertex vertex(int id) {
        if (vertexData != null) {
            return (SFT_Vertex) vertexData.get(id);
        } else {
            return vertices[id];
        }
    }

    public SFT_Triangle triangle(int id) {
        if (triangleData != null) {
            return (SFT_Triangle) triangleData.get(id);
        } else {
            return triangles[id];
        }
    }
     ...
     * @param newVertex
     */
    public void addVertex(SFT_Vertex newVertex) {
        newVertex.ID = vertexData.size();
        vertexData.add(newVertex);
    }

    public void addVertex(float x, float y, float z) {
        addVertex(new SFT_Vertex(x, y, z));
    }

    public void addTriangle(SFT_Triangle newTriangle) {
    ...
    public void addTriangle(SFT_Vertex a, SFT_Vertex b, SFT_Vertex c) {
        addTriangle(new SFT_Triangle(a, b, c));
    }

    public void removeVertex(SFT_Vertex v) {
        vertexData.remove(v);
    }

    public void removeTriangle(SFT_Triangle t) {
    ...
    public void rebuild() {
        ...
        vertices = new SFT_Vertex[numVertices];
        ...
        triangles = new SFT_Triangle[numTriangles];
        for (int i = 0; i < numTriangles; i++) {
            triangles[i] = tri = (SFT_Triangle) triangleData.get(i);
            tri.ID = i;
    ...
    public void rebuild_OLD() {
        ...
        vertices = new SFT_Vertex[numVertices];
        for (int i = 0; i < numVertices; i++) {
            vertices[i] = (SFT_Vertex) vertexData.get(i);
        }
        ...
        triangles = new SFT_Triangle[numTriangles];
        for (int i = 0; i < numTriangles; i++) {
            triangles[i] = (SFT_Triangle) triangleData.get(i); //enum.nextElement();
            triangles[i].ID = i;
        }
     ...
     */
    public void registerSmoothNeighbors(ArrayList neighborTris, SFT_Vertex v, SFT_Triangle t) {
        SFT_Triangle neighborTri;
        ...
        for (int i = 0; i < v.neighborTris.size(); i++) {
            neighborTri = (SFT_Triangle) v.neighborTris.get(i);
            // Test for > 90 degree angle between triangle t and this triangle
            if (SFT_Triangle.onSameSurface(t, neighborTri, cos_angle)) {
     ...
     */
    public SFT_Vector min() {
        if (numVertices == 0) {
            return new SFT_Vector(0f, 0f, 0f);
        }
        ...
        return new SFT_Vector(minX, minY, minZ);
    }

    /**
     * Return maximum point in the mesh.
     */
    public SFT_Vector max() {
        if (numVertices == 0) {
            return new SFT_Vector(0f, 0f, 0f);
        }
        ...
        return new SFT_Vector(maxX, maxY, maxZ);
    }

    /**
     * Return the center point of the mesh.
     */
    public SFT_Vector getCenter() {
        SFT_Vector max = max();
        SFT_Vector min = min();
        return new SFT_Vector((max.x + min.x) / 2f, (max.y + min.y) / 2f, (max.z + min.z) / 2f);
    }

    /**
     * Returns the dimensions of this mesh.
     */
    public SFT_Vector getDimension() {
        SFT_Vector max = max();
        SFT_Vector min = min();
        return new SFT_Vector(max.x - min.x, max.y - min.y, max.z - min.z);
    }

    /**
     * return the vertex array
     */
    public SFT_Vertex[] getVertexArray() {
        return vertices;
    }
     ...
     * @see  sortTriangles()
     */
    public void projectVerts(SFT_Mesh obj, FloatBuffer modelMatrix, FloatBuffer projectionMatrix, IntBuffer viewport) {
        SFT_Vertex v;
     ...
     */
    public SFT_Triangle[] sortTriangles(SFT_Triangle[] tri, int L, int R) {
     ...
     * @see projectVerts()
     */
    public SFT_Triangle getTriangle(float x, float y) {
     ...
     * @see projectVerts()
     */
    public static SFT_Triangle getTriangle(float x, float y, SFT_Triangle[] _triangles) {
        ArrayList candidates = new ArrayList();
        SFT_Triangle closestT = null;
        SFT_Triangle t;
        float minZ = 100000;
        ...
        for (int j = 0; j < candidates.size(); j++) {
            if (((SFT_Triangle) (candidates.get(j))).Zdepth < minZ) {
                closestT = (SFT_Triangle) (candidates.get(j));
     ...
     */
    public SFT_Mesh makeClone() {
        SFT_Mesh clone = new SFT_Mesh();
        SFT_Triangle ct;  // cloned triangle
    ...
    public String toString() {
        String out = "<object id=" + name + ">\r\n";
        for (int i = 0; i < numVertices; i++) {
            out += vertices[i].toString();
        }
        return out;
    }
    ...
    public SFT_Triangle[] getGroupFaces(int g) {
        return groupFaces[g];
    }
    ...
    public void makeGroups(int num) {
        groupFaces = new SFT_Triangle[num][];
    ...
    public void initGroup(int groupNum, String name, String materialName, int numTriangles) {
        groupFaces[groupNum] = new SFT_Triangle[numTriangles];
     ...
     * @see setGroup()
     */
    public void addTriangle(SFT_Triangle newTriangle, int groupNum, int triangleNum) {
