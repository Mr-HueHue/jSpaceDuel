 ...
 * @author JJ
 */
public class SFT_MaterialLibrary {
    ...
    SFT_Material[] materials;


    public SFT_MaterialLibrary(String mtlFilename) {
    ...
    public SFT_Material[] loadMaterials(String mtlFilename) {
        SFT_Material[] mtls = null;
        // Separate leading path from filename (we'll load textures from same folder)
        String[] pathParts = SFT_Util.getPathAndFile(mtlFilename);
        filepath = pathParts[0];
        filename = pathParts[1];
        // read the mtl file
        try {
        	mtls = loadMaterials(new BufferedReader(new InputStreamReader(new FileInputStream(mtlFilename))));
        }
        catch (Exception e) {
     ...
     * </PRE>
     */
    public SFT_Material[] loadMaterials(BufferedReader br) {
    	ArrayList mtlslist = new ArrayList();
    	SFT_Material material = null;
    	String line = "";
    		...
    		while ((line = br.readLine()) != null) {
    			...
    			if (line.length() > 0) {
    				if (line.startsWith("#")) {
    					...
    					// newmtl some_name
    					material = new SFT_Material();  // start new material
    			        ...
    			        if (textureFile != null && !textureFile.equals("")) {
        					int textureHandle = 0;
    			        	try {
    					    	textureHandle = SFT_Util.makeTexture(filepath + textureFile);
    			        	}
    			        	catch (Exception e) {
    	...
    	SFT_Material[] mtls = new SFT_Material[ mtlslist.size() ];
     ...
     */
    public void writeLibe(SFT_Material[] mtls, String filename) {
    	try {
    ...
    public void writeLibe(SFT_Material[] mtls, PrintWriter out) {
    	if (out != null) {
    		out.println("#");
     ...
     *  Write one material.
     */
    public void write(PrintWriter out, SFT_Material mtl)
    {
    	if (out != null) {
     ...
     */
    public SFT_Material getClone(SFT_Material mtl) {
    	SFT_Material clone = new SFT_Material();
     ...
     */
    public SFT_Material find(String materialName) {
