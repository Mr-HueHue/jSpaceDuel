 ...
 */
public class SFT_Triangle {

    public SFT_Vertex p1;  // first  vertex
    public SFT_Vertex p2;  // second vertex
    public SFT_Vertex p3;  // third  vertex
    public SFT_Vector norm1; // normal at vert 1
    public SFT_Vector norm2; // normal at vert 2
    public SFT_Vector norm3; // normal at vert 3
    public SFT_Vector uvw1 = new SFT_Vector();
    ; // texture coord at vert 1
        public SFT_Vector uvw2 = new SFT_Vector();
    ; // texture coord at vert 2
        public SFT_Vector uvw3 = new SFT_Vector();
    ; // texture coord at vert 3
    ...
    public SFT_Vector n = new SFT_Vector();  // Normal vector of flat triangle
    public float Zdepth = 0f;               // screen Z depth
    public int ID = 0;
    public int groupID = 0;
    int materialID;  // index into materials array

    public SFT_Triangle(SFT_Vertex a, SFT_Vertex b, SFT_Vertex c) {
        p1 = a;
        p2 = b;
        p3 = c;
    }
    ...
    public void recalcFaceNormal() {
        n = SFT_Vector.getNormal(p1.pos, p2.pos, p3.pos);
    }
     ...
     */
    public SFT_Vector recalcVertexNormal(ArrayList neighbors) {
        float nx = 0;
        float ny = 0;
        float nz = 0;
        SFT_Triangle tri;
        SFT_Vector wn = new SFT_Vector();
        // for each neighbor triangle, average the normals
        for (int i = 0; i < neighbors.size(); i++) {
            tri = (SFT_Triangle) neighbors.get(i);
            wn = tri.getWeightedNormal();
            nx += wn.x;
            ny += wn.y;
            nz += wn.z;
        }
        SFT_Vector vertn = new SFT_Vector(nx, ny, nz);
        vertn.normalize();
        return vertn;
    }

    public SFT_Vector getWeightedNormal() {
        return SFT_Vector.vectorProduct(p1.pos, p2.pos, p3.pos);
    }
    ...
    public SFT_Vector getCenter() {
        ...
        return new SFT_Vector(cx, cy, cz);
    }
     ...
     */
    public static boolean onSameSurface(SFT_Triangle t1, SFT_Triangle t2, float cos_angle) {
        float dot = SFT_Vector.dotProduct(t1.n, t2.n);
    ...
    public SFT_Triangle makeClone() {
        SFT_Triangle clone = new SFT_Triangle(p1.makeClone(), p2.makeClone(), p3.makeClone());
