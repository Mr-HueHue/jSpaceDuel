 ...
 * @author JJ
 */
public class SFT_Model {
    // a default material to use if none is specified
    public static SFT_Material defaultMtl = new SFT_Material();

    public SFT_Mesh mesh;                 // holds vertex, triangle and group information
    public int displayListID;

    // !!!! this will be null. NEED to check this in OBJ_Reader and check renderGroups() (BROKEN!!!)
    public SFT_Material[] groupMaterials;  // holds material for each face group (1 group to 1 material)

	public SFT_Model(String filename) {
		// load OBJ file
	 ...
	 */
    public final SFT_Mesh loadMesh(String filename) {
    	if (filename.toUpperCase().endsWith(".OBJ")) {
    		SFT_OBJImporter importer = new SFT_OBJImporter();
    		mesh = importer.load(filename);
    	}
    	else {
    		SFT_3DSImporter importer = new SFT_3DSImporter();
    		mesh = importer.load(filename);
	...
	public void renderGroup(String groupName)
	{
		...
		SFT_Material[] materials = mesh.materials;   // loaded from the .mtl file
		SFT_Triangle[] triangles = mesh.getGroupFaces(GID);  // each group may have a material
		SFT_Material mtl;
		SFT_Triangle t;
		int currMtl = -1;
		int i = 0;
     ...
     */
    public void render(SFT_Mesh m)
    {
        SFT_Material[] materials = m.materials;   // loaded from the .mtl file
        SFT_Material mtl;
        SFT_Triangle t;
        int currMtl = -1;
        int i = 0;
