...
public class SFT_Image {
     ...
     * @param imgName
     */
    public SFT_Image(String imgName) {
        BufferedImage img = loadJavaImage(imgName);
        if (makeGLImage(img, true, false)) {
            System.out.println("SFT_Image(String): loaded " + imgName + ", width=" + w + " height=" + h);
        }
    }
     ...
     * @param imgName
     */
    public SFT_Image(String imgName, boolean flipYaxis, boolean convertPow2) {
        BufferedImage img = loadJavaImage(imgName);
        if (makeGLImage(img, flipYaxis, convertPow2)) {
            System.out.println("SFT_Image(String,bool,bool): loaded " + imgName + ", width=" + w + " height=" + h);
        }
    }
     ...
     * @param h
     */
    public SFT_Image(byte[] bytes) {
        BufferedImage img = makeBufferedImage(bytes);
        if (makeGLImage(img, true, false)) {
            System.out.println("SFT_Image(byte[]): loaded image from bytes[" + bytes.length + "]");
        } else {
            System.err.println("SFT_Image(byte[]): could not create Image from bytes[" + bytes.length + "]");
        }
    }
     ...
     * @param h
     */
    public SFT_Image(byte[] bytes, boolean flipYaxis, boolean convertPow2) {
        BufferedImage img = makeBufferedImage(bytes);
        if (makeGLImage(img, flipYaxis, convertPow2)) {
            System.out.println("SFT_Image(byte[],bool,bool): loaded image from bytes[" + bytes.length + "]");
        } else {
            System.out.println("SFT_Image(byte[],bool,bool): could not create Image from bytes[" + bytes.length + "]");
        }
    }
     ...
     */
    public final BufferedImage makeBufferedImage(byte[] imageFileContents) {
        BufferedImage bi = null;
        try {
            InputStream in = new ByteArrayInputStream(imageFileContents);
            bi = javax.imageio.ImageIO.read(in);
        } catch (IOException ioe) {
            System.err.println("SFT_Image.makeBufferedImage(): " + ioe);
        }
        return bi;
    }
     ...
     * @param h
     */
    public SFT_Image(ByteBuffer gl_pixels, int w, int h) {
        if (gl_pixels != null) {
    ...
    public final boolean makeGLImage(BufferedImage tmpi, boolean flipYaxis, boolean convertToPow2) {
        if (tmpi != null) {
            ...
            textureW = SFT_Util.getPowerOfTwoBiggerThan(w); // the texture size big enough to hold this image
            textureH = SFT_Util.getPowerOfTwoBiggerThan(h); // the texture size big enough to hold this image
     ...
     */
    public final BufferedImage loadJavaImage(String imgName) {
        BufferedImage tmpi = null;
        try {
            tmpi = ImageIO.read(new FileInputStream(imgName));
        } catch (Exception e) {
            System.err.println("SFT_Image.loadJavaImage() exception: FAILED TO LOAD IMAGE " + e);
        }
        return tmpi;
    }
    ...
    public static ByteBuffer convertImagePixelsARGB(int[] jpixels, int imgw, int imgh, boolean flipVertically) {
        ...
        ByteBuffer bb = SFT_Util.allocBytes(jpixels.length * 4);  // 4 bytes per pixel
    ...
    public static void savePixelsToPNG(int[] pixels, int width, int height, String imageFilename, boolean flipY) {
        if (pixels != null && imageFilename != null) {
            if (flipY) {
                ...
                pixels = SFT_Image.flipPixels(pixels, width, height);
            }
            try {
                ...
                System.err.println("SFT_Image.savePixelsToPNG(" + imageFilename + "): exception " + e);
            }
        }
    }
    ...
    public static BufferedImage convertToPowerOf2(BufferedImage bsrc) {
        ...
        int newW = SFT_Util.getPowerOfTwoBiggerThan(bsrc.getWidth());
        int newH = SFT_Util.getPowerOfTwoBiggerThan(bsrc.getHeight());
