...
/**
 * SFT_Font uses the Java Font class to create character sets and render text.  
 * SFT_Font can generate text in nearly any size, and in any font that Java supports.  
 * <P>
 */
public class SFT_Font {
     ...
     */
    public SFT_Font(Font f, float[] fgColor, float[] bgColor) {
    ...
    public int makeFontTexture(Font f, float[] fgColor, float[] bgColor) {
        ...
        try {
            ...
            int[] pixelsARGB = SFT_Image.getImagePixels(image);
            texture = SFT_Util.makeTexture(pixelsARGB, image.getWidth(), image.getHeight(), false);
        } catch (Exception e) {
    ...
    public static BufferedImage createCharImage(String text, Font font, float[] fgColor, float[] bgColor) {
        ...
        int imgSize = SFT_Util.getPowerOfTwoBiggerThan(maxCharSize);
        if (imgSize > 2048) {
            System.err.println("SFT_Font.createCharImage(): texture size will be too big (" + imgSize + ") Make the font size smaller.");
            return null;
        }
    ...
    public BufferedImage createFontImage(Font font, float[] fgColor, float[] bgColor) {
        ...
        textureSize = SFT_Util.getPowerOfTwoBiggerThan(fontSize * 10);
        System.out.println("SFT_Font.getFontImageSize(): build font with fontsize=" + fontSize + " gridsize=" + (fontSize * 10) + " texturesize=" + textureSize);
        if (textureSize > 2048) {
            System.err.println("SFT_Font.createFontImage(): texture size will be too big (" + textureSize + ") Make the font size smaller.");
            return null;
        }
